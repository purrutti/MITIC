...
ModbusRtu master(0, 3, 46); // MDUINO
...
void save(int startAddress) {
    ...
    address = condition.save();
    for (int i = 0; i < 2; i++) {
        regulPression[i].startAddress = address;
        address = regulPression[i].save(address);
    }
    regulTempEC.startAddress = address;
    address = regulTempEC.save(address);
}

void load(int startAddress) {
    int address = startAddress;

    condition.startAddress = address;
    address = condition.load();
    for (int i = 0; i < 2; i++) {
        regulPression[i].startAddress = address;
...
void setup() {
    ...
    */
    ...
    condition.Meso[0] = Mesocosme(PIN_DEBITMETRE_0, 0, PIN_NIVEAU_L_0, PIN_NIVEAU_LL_0, 0);
    condition.Meso[1] = Mesocosme(PIN_DEBITMETRE_1, 0, PIN_NIVEAU_L_1, PIN_NIVEAU_LL_1, 1);
    condition.Meso[2] = Mesocosme(PIN_DEBITMETRE_2, 0, PIN_NIVEAU_L_2, PIN_NIVEAU_LL_2, 2);
    condition.Meso[0].debit = 0;
    condition.Meso[1].debit = 0;
    condition.Meso[2].debit = 0;

    regulPression[0] = Regul();
    regulPression[1] = Regul();
    regulTempEC = Regul();
    regulTempEC.sortiePID_pc = 0;

    regulPression[0].sortiePID_pc = 0;
    regulPression[1].sortiePID_pc = 0;

    load(2);

        condition.condID = 0;
        for (uint8_t j = 0; j < 3; j++) condition.Meso[j] = Mesocosme(j);

    tempoSensorRead.interval = (unsigned long)200;//50
    tempoRegul.interval = (unsigned long)100;
    tempoCheckMeso.interval = (unsigned long)200;
    tempoSendValues.interval = (unsigned long)5000;
    tempowriteSD.interval = (unsigned long)5000 * 60;
    tempoCheckSun.interval = (unsigned long)60 * 1000;
    tempoSendParams.interval = (unsigned long)5000;
    tempoCheckWaterLevelCO2.interval = (unsigned long)1000;
    tempoRR.interval = (unsigned long)10000;
  ...
  // so you have to close this one before opening another.
        ...
        if (!SD.exists(path)) {
            ...
            if (dataFile) {
                dataFile.print(F("timestamp;Sun;Tide;Ambiant.O2;Ambiant.Conductivity;Ambiant.Turbidity;Ambiant.Fluo;Ambiant.Temperature;Ambiant.pH;"));
                    ...
                    String header = F("Condition.Temperature;");
                    header += F("Condition.pH;");
                    header += F("Condition.consigne_pH;");
                    header += F("Condition.sortiePID_pH;");
                    dataFile.print(header);
                    for (int j = 0; j < 3; j++) {
                        header = F("Condition.Meso["); header += j; header += F("].Temperature;"); dataFile.print(header);
                        header = F("Condition.Meso["); header += j; header += F("].pH;"); dataFile.print(header);
                        header = F("Condition.Meso["); header += j; header += F("].FlowRate;"); dataFile.print(header);
                        header = F("Condition.Meso["); header += j; header += F("].LevelH;"); dataFile.print(header);
                        header = F("Condition.Meso["); header += j; header += F("].LevelL;"); dataFile.print(header);
                        header = F("Condition.Meso["); header += j; header += F("].LevelLL;"); dataFile.print(header);
                    }
        ...
        if (dataFile) {
            ...
            dataFile.print(RTC.getTime()); dataFile.print(sep); dataFile.print(masterData.currentSun); dataFile.print(sep); dataFile.print("0"); dataFile.print(sep); dataFile.print(masterData.oxy); dataFile.print(sep); dataFile.print(masterData.cond); dataFile.print(sep); dataFile.print(masterData.turb); dataFile.print(sep); dataFile.print(masterData.fluo); dataFile.print(sep); dataFile.print(masterData.temperature); dataFile.print(sep); dataFile.print(masterData.pH); dataFile.print(sep);
                ...
                dataFile.print(condition.mesureTemperature); dataFile.print(sep);
                dataFile.print(condition.mesurepH); dataFile.print(sep);
                dataFile.print(condition.regulpH.consigne); dataFile.print(sep);
                dataFile.print(condition.regulpH.sortiePID_pc); dataFile.print(sep);


                for (int j = 0; j < 3; j++) {
                    dataFile.print(condition.Meso[j].temperature); dataFile.print(sep);
                    dataFile.print(condition.Meso[j].pH); dataFile.print(sep);
                    dataFile.print(condition.Meso[j].debit); dataFile.print(sep);
                    dataFile.print(condition.Meso[j].alarmeNiveauHaut); dataFile.print(sep);
                    dataFile.print(condition.Meso[j].alarmeNiveauBas); dataFile.print(sep);
                    dataFile.print(condition.Meso[j].alarmeNiveauTresBas); dataFile.print(sep);
                }
            dataFile.println("");
            dataFile.close();
            // print to the serial port too:
...
int regulationpH() {
    ...
    if (condition.regulpH.autorisationForcage) {
        if (condition.regulpH.consigneForcage > 0 && condition.regulpH.consigneForcage <= 100) {
            dutyCycle = condition.regulpH.consigneForcage;
        }
        else {
        ...
        if (condition.mesurepH <= condition.regulpH.consigne) {
            condition.regulpH.sortiePID = 0;
        }
        else  condition.regulpH.pid.Compute();

        condition.regulpH.sortiePID_pc = (int)condition.regulpH.sortiePID;

        dutyCycle = condition.regulpH.sortiePID;
    }
...
void readJSON(char* json) {
   ...
   if (senderID == 4) {
    ...
    if (condID != CONDID) return;
    switch (command) {
    case 0:
        condition.load();
        Serial.println("REQ PARAMS");
        condition.serializeParams(buffer, heure, 0);
        webSocket.sendTXT(buffer);
        break;
    case REQ_DATA:

        Serial.println("REQ DATA");
        //condition.serializeData(heure, 0, buffer); 
        condition.serializeData(buffer, heure, 0);
        webSocket.sendTXT(buffer);
    ...
    case SEND_PARAMS:
        ...
        Serial.print("Received params");Serial.println(json);
        condition.save();
        //if (condID > 0) webSocket.broadcastTXT(buffer); //On forward le parametrage
        if (senderID == 4) {
            //condition.serializeParams(heure, 0, buffer);
            condition.serializeParams(buffer, heure, 0);
            
        Serial.print("Saved params");Serial.println(buffer);
            //webSocket.sendTXT(buffer);
    ...
    /*case SEND_DATA:
        ...
        if (condID > 0) {
            condition.regulpH.consigne = condition.regulpH.offset + masterData.pH;
            condition.regulTemp.consigne = condition.regulTemp.offset + masterData.temperature;
        }
        //condition.serializeParams(heure, 0, buffer);
        condition.serializeParams(buffer, heure, 0);
        webSocket.sendTXT(buffer);
    ...
    case REQ_MASTER_DATA:
        ...
        Serial.println("REQ MASTER DATA");
...
void sendData() {
        ...
        char buf[600];
        condition.serializeData(buf, RTC.getTime(), CONDID);
        Serial.println(buf);
...
void sendMasterData() {
    ...
    char buf[600];
        SerializeMasterData(RTC.getTime(), buf);
        Serial.println(buf);
        webSocket.sendTXT(buf);
...
bool SerializeMasterData(uint32_t timeString, char* buffer) {
    ...
    doc[oxy] = String(masterData.oxy,2);
    doc[cond] = String(masterData.cond,2);
    ...
    doc[F("tempPAC")] = String(masterData.tempPAC,2);
